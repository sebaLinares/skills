...
Read the local cached reference for complete styles, edge routing, containers, layers, tags, metadata, and dark mode rules:

`~/.claude/skills/drawio/references/xml-reference.md`

Use the Read tool directly on that path — do NOT fetch the URL or spawn a sub-agent. The file is kept manually up to date (update it every few months from https://raw.githubusercontent.com/jgraph/drawio-mcp/main/shared/xml-reference.md if needed).
